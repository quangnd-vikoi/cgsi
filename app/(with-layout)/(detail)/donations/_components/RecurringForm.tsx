...
const RecurringForm = ({
    ...
    const getDefaultAccountNo = useTradingAccountStore(
        (state) => state.getDefaultAccountNo,
    );
        ...
        try {
            ...
            if (response.success) {
                setRecurringDonations(
                    recurringDonations.filter((item) => item.id !== plan.id),
                );
                toast.success(
                    "Donation Cancelled",
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            ...
            <div className="space-y-6 pad-x flex-shrink-0">
                <div className="space-y-4 text-sm font-normal text-typo-secondary mt-6">
                    <p className="md:hidden">
                        Automatically donate a portion every time you execute a
                        trade within your chosen time period.
                    </p>
                    <p className="hidden md:inline-block">
                        Automatically donate a fixed amount every time you
                        execute a trade within your chosen time period. You may
                        view your donation transactions on your monthly account
                        statements.
                    </p>
                </div>
                    ...
                    <Dialog open={open} onOpenChange={setOpen}>
                        ...
                        <DialogContent className="sm:max-w-[530px] p-0 max-h-[730px] flex flex-col gap-0 ">
                            ...
                            <div className="flex flex-col gap-4 pad-x text-xs md:text-sm font-normal text-typo-secondary max-h-[450px] md:max-h-none overflow-auto my-4 md:my-6">
                                <p>
                                    Your recurring donation will remain active
                                    for the selected duration. During this
                                    period, each trade you make will
                                    automatically contribute the chosen donation
                                    amount.
                                </p>
                            </div>
                            ...
                            {SELECT_FIELDS.map((field) => {
                                const fieldHasError =
                                    showValidationErrors && hasError(field.id);
                                return (
                                    <div key={field.id} className="pad-x mb-6">
                                        ...
                                        <Select
                                            value={
                                                formValues[
                                                    field.id as keyof typeof formValues
                                                ]
                                            }
                                            onValueChange={(value) =>
                            ...
                            <DialogFooter className="justify-end rounded-b-lg pad-x flex-row">
                                <Button
                                    type="button"
                                    onClick={handleSetup}
                                    className="px-3 py-2 rounded-sm font-normal text-sm md:text-base"
            ...
            <div className="pad-x flex-shrink-0">
                ...
                <p className="md:hidden text-base font-semibold text-typo-primary">
                    Recurring Donation List
                </p>
            </div>
            {isLoadingPlans ? (
                ...
                <div className="mt-6 flex-1 pad-x flex flex-col gap-6 justify-center items-center">
                    ...
                    <p className="text-sm font-normal text-typo-tertiary text-center px-2">
                        There are currently no active recurring donations. Setup
                        one now to support our cause!
                    </p>
                </div>
            ) : (
                                        ...
                                        <Alert
                                            ...
                                            description={
                                                <p>
                                                    ...
                                                    Cancel your recurring
                                                    donation? This action will
                                                    permanently stop all future
