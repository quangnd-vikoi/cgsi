...
const MarketData = () => {
    ...
    useEffect(() => {
        ...
        return () =>
            window.removeEventListener("market-data:reset", handleReset);
    }, [loadCatalog]);

    // Pre-filter groups for each tab
    const nonProResearch = catalog
        ? filterByProfessional(catalog.research, false)
        : [];
    const nonProMarketData = catalog
        ? filterByProfessional(catalog.marketData, false)
        : [];
    const proResearch = catalog
        ? filterByProfessional(catalog.research, true)
        : [];
    const proMarketData = catalog
        ? filterByProfessional(catalog.marketData, true)
        : [];

    const [agreements, setAgreements] = useState<ISubscriptionAgreement[]>([]);
    const [agreementContents, setAgreementContents] = useState<
        Record<string, ISubscriptionAgreementContent>
    >({});
    const [extendedData, setExtendedData] =
        useState<IMarketSubscriptionExtendedData>({
            name: "",
            address: "",
            occupation: "",
            value_01: true,
    ...
    const fetchDeclarationInfo = useCallback(async () => {
        ...
        try {
            const res = await fetchAPI<IDeclarationInfoResponse>(
            ...
            if (res.success && res.data) {
                setExtendedData((prev) => ({
                    ...
                    employerAddress:
                        res.data!.employerAddress ?? prev.employerAddress,
                }));
            }
        } finally {
            ...
            if (res.success && res.data) {
                ...
                if (allAgreementIds.length > 0) {
                    const contentResults = await Promise.all(
                        allAgreementIds.map((id) =>
                            getMarketDataAgreementContent(id),
                        ),
                    );
